...
<main>
    @if($workspace->quota)
        <h2 class="my-4">
            This month max limit: {{ $workspace->quota->limit }} $
        </h2>
    @endif
    @if($workspace->quota)
        <form action="{{ route('workspace.quota.destroy', [$workspace->id, $workspace->quota->id]) }}" method="POST">
            @csrf
            @method('delete')
            <button class="my-4 btn btn-danger">
                Delete Quota
            </button>
        </form>
        @else
        <form class="mx-auto text-center d-flex flex-column gap-4 w-50" action="{{ route('workspace.quota.store', $workspace->id) }}" method="POST">
            @csrf
            <h2>Create Quota</h2>
    ...
    @endif
        @error('message')
            <span class="d-block alert alert-success">
                {{ $message }}
            </span>
        @enderror
